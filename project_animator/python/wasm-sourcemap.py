...
def strip_debug_sections(wasm):
  ...
  while pos < len(wasm):
    ...
    if section_id == 0:
      ...
      if name == "linking" or name == "sourceMappingURL" or name.startswith("reloc..debug_".encode('ascii')) or name.startswith(".debug_".encode('ascii')):
...
def append_source_mapping(wasm, url):
  ...
  section_content = encode_uint_var(len(section_name)) + section_name.encode('ascii') + encode_uint_var(len(url)) + url.encode('ascii')
  #print('section_content %s' % (wasm + encode_uint_var(0) + encode_uint_var(len(section_content)) + section_content))
