...
void ImGui::SeparatorTextEx(ImGuiID id, const char* label, const char* label_end, float extra_w)
{
    ...
    const ImVec2 label_size = (label == 0 && label_end == 0)? ImVec2() : CalcTextSize(label, label_end, false);
    const ImVec2 pos = window->DC.CursorPos;
    ...
    else if (held && !tab_appearing && IsMouseDragging(0))
    {
        ...
        if (can_undock)
        {
            ...
            if (undocking_tab)
            {
                ...
                g.ActiveIdClickOffset = g.ActiveIdClickOffset - (g.MovingWindow->Pos - bb.Min);
