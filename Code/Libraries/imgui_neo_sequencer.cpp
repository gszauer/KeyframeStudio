...
namespace ImGui
{
    ...
    struct ImGuiNeoSequencerInternalData
    {
        ...
        bool IsLastTimelineRightClicked = false;
        bool IsLastTimelineLeftClicked = false;

        bool IsLastGroupLeftClicked  = false;
        bool IsLastGroupRightClicked = false;
        ...
        bool IsLastKeyframeLeftClicked = false;
    ...
    static void processCurrentFrame(FrameIndexType* frame, ImGuiNeoSequencerInternalData& context)
    {
        ...
        pointerRect.Min = pointerRect.Min - ImVec2{2.0f, 2.0f};
        pointerRect.Max = pointerRect.Max + ImVec2{2.0f, 2.0f};
    ...
    static bool createKeyframe(int32_t* frame)
    {
        ...
        context.IsLastKeyframeLeftClicked = hovered && IsMouseClicked(ImGuiMouseButton_Left);
...
//
    ...
    static bool groupBehaviour(const ImGuiID id, bool* open, const ImVec2 labelSize)
    {
        ...
        if (addGroupRes)
        {
            context.IsLastGroupLeftClicked = IsItemClicked(ImGuiMouseButton_Left);
            context.IsLastGroupRightClicked = IsItemClicked(ImGuiMouseButton_Right);
            if (context.IsLastGroupLeftClicked || context.IsLastGroupRightClicked)
            {
    ...
    static bool timelineBehaviour(const ImGuiID id, const ImVec2 labelSize)
    {
        ...
        const auto addGroupRes = ItemAdd(groupBB, id);
        if (addGroupRes) {
            context.IsLastTimelineRightClicked = IsItemClicked(ImGuiMouseButton_Right);
            context.IsLastTimelineLeftClicked = IsItemClicked(ImGuiMouseButton_Left);
            if (context.IsLastTimelineRightClicked || context.IsLastTimelineLeftClicked)
            {
                context.LastSelectedTimeline = context.SelectedTimeline;
                context.SelectedTimeline = context.SelectedTimeline == id ? 0 : id;
            }
            ...
            context.IsLastTimelineRightClicked = false;
            context.IsLastTimelineLeftClicked = false;
        }
    ...
    bool BeginNeoTimelineEx(const char* label, bool* open, ImGuiNeoTimelineFlags flags)
    {
        ...
        const auto& imStyle = GetStyle();
        ImGuiWindow* window = GetCurrentWindow();
        const ImGuiID id = (ImGuiID)((size_t)label);// window->GetID(label);
    ...
    bool IsNeoKeyframeLeftClicked()
    {
        IM_ASSERT(inSequencer && "Not in active sequencer!");
        auto& context = sequencerData[currentSequencer];

        return context.IsLastKeyframeLeftClicked;
    }

    void DeselectNeoTrack() {
        IM_ASSERT(inSequencer && "Not in active sequencer!");
        auto& context = sequencerData[currentSequencer];

        context.SelectedTimeline = 0;
    }

    bool IsNeoTimelineRightClicked() {
        IM_ASSERT(inSequencer && "Not in active sequencer!");
        auto& context = sequencerData[currentSequencer];

        return context.IsLastTimelineRightClicked;
    }

    bool IsNeoTimelineLeftClicked() {
        IM_ASSERT(inSequencer && "Not in active sequencer!");
        auto& context = sequencerData[currentSequencer];

        return context.IsLastTimelineLeftClicked;
    }

    bool IsNeoGroupRightClicked() {
        IM_ASSERT(inSequencer && "Not in active sequencer!");
        auto& context = sequencerData[currentSequencer];

        return context.IsLastGroupRightClicked;
    }

    bool IsNeoGroupLeftClicked() {
        IM_ASSERT(inSequencer && "Not in active sequencer!");
        auto& context = sequencerData[currentSequencer];

        return context.IsLastGroupLeftClicked;
    }

    

    ImGuiID GetNeoLastTimeline() {
        IM_ASSERT(inSequencer && "Not in active sequencer!");
        auto& context = sequencerData[currentSequencer];

        return context.LastSelectedTimeline;
    }

    ImGuiID GetNeoThisTimeline() {
        IM_ASSERT(inSequencer && "Not in active sequencer!");
        auto& context = sequencerData[currentSequencer];
